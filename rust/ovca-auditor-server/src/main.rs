...
fn matches_auditor_path(path: &Path) -> bool {
    let text = path
        .file_name()
        .and_then(|name| name.to_str())
        .unwrap_or("")
        .to_ascii_lowercase();
    text.contains("auditor") || text.contains("cross-audit") || text.contains("cross_audit")
}
...
fn collect_candidates(root: &Path) -> Vec<EvidenceCandidate> {
    ...
    for dir in candidate_roots(root) {
        ...
        for entry in entries.flatten() {
            let path = entry.path();
            if !path.is_dir() || !matches_auditor_path(&path) {
                continue;
            }
            let modified = entry
                .metadata()
...
fn latest_from_candidate(root: &Path, candidate: &EvidenceCandidate) -> Value {
    ...
    let audit_path = first_existing(&[
        candidate.path.join("result.md"),
        candidate.path.join("audit.md"),
    ...
    if summary.is_empty() {
        summary = status_payload
            ...
            .unwrap_or_else(|| {
                "Auditor cross-audit artifact found without a readable summary.".to_string()
            });
    }
...
fn auditor_cross_audit_status(root: &Path, args: Value) -> Value {
    let query = args
        .get("query")
        .and_then(Value::as_str)
        .unwrap_or("")
    ...
    let latest = candidates
        ...
        .unwrap_or_else(|| {
            json!({
                ...
                "status": "unknown",
                "summary": "No Auditor cross-audit artifacts found in tasks/audits, tasks/outbox, or tasks/inbox.",
                "source": Value::Null,
                "artifact": Value::Null,
                "status_payload": {},
            })
    ...
    let history = candidates
        ...
        .collect::<Vec<_>>();
    let risk_summary = latest
        .get("summary")
        .and_then(Value::as_str)
        .unwrap_or("No Auditor cross-audit evidence available.")
        .to_string();
...
fn build_router(root: PathBuf) -> axum::Router {
    ...
    let (router, _state) = McpServer::builder("oracle-auditor-mcp", "auditor")
        .tool(
            "auditor_cross_audit_status",
            "Read Auditor cross-audit/compliance status from task audit artifacts.",
            schema,
            move |args: Value| {
                let root = root.clone();
                Box::pin(async move { auditor_cross_audit_status(&root, args) }) as BoxFuture<Value>
            },
        )
        .into_router();
    router
...
async fn main() -> Result<()> {
    ...
    let default_port = port_from_env("MCP_AUDITOR_PORT", DEFAULT_PORT);
    let (port, root) = parse_args(default_port);
    info!(port, root = %root.display(), "oracle-auditor-server starting");
...
mod tests {
    ...
    fn seed_root() -> TempDir {
        ...
        let dir = temp
            ...
            .join("tasks")
            .join("audits")
            .join("demo__p02__auditor__cross-audit");
        std::fs::create_dir_all(&dir).expect("create dir");
        std::fs::write(
            dir.join("status.json"),
            r#"{"overall_status":"ok","summary":"Auditor cross-audit passed"}"#,
        )
        .expect("write status");
        std::fs::write(
            dir.join("result.md"),
            "Auditor cross-audit result: no open risks.",
        )
        .expect("write result");
        temp
    }
    ...
    fn status_tolerates_missing_artifacts() {
        ...
        let payload = auditor_cross_audit_status(root.path(), json!({}));
        assert_eq!(payload["ok"], true);
        assert_eq!(payload["overall_status"], "unknown");
        assert_eq!(payload["latest"]["overall_status"], "unknown");
        assert!(payload["risks"].is_array());
    }

    #[test]
    fn status_reads_latest_artifact() {
        let root = seed_root();
        let payload = auditor_cross_audit_status(root.path(), json!({"limit_history": 5}));
        assert_eq!(payload["ok"], true);
    ...
    #[tokio::test]
    async fn router_registers_auditor_tool() {
        let root = seed_root();
        ...
        assert!(tools
            .iter()
            .any(|tool| tool["name"] == "auditor_cross_audit_status"));
    }

    #[tokio::test]
    async fn router_calls_auditor_tool() {
        let root = seed_root();
        let payload = json!({"name": "auditor_cross_audit_status", "arguments": {"query": "demo"}});
        let req = http::Request::builder()
            .method("POST")
            .uri("/tools/call")
